...
- Backend RAG (stub): added `/api/rag/ingest` (saves to `Transcripts/Inbox` with manifest, duplicate hash returns status=duplicate) and `/api/rag/inbox` (reads manifest). Scaffolded `backend/Transcripts/` for storage.
