...
- **Sync flow:** Local branch → `python3 -m mkdocs build --clean` (or `tools/docs_build.sh`) → optional preview `tools/docs_serve.sh` on port 9003 → merge to `main` → CI `.github/workflows/docs.yml` deploys to `http://20.217.86.4:8000` via docker `docs` service.
