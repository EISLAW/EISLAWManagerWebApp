  ...
  return (
    <div className="card">
      <div className="flex gap-2 items-end">
        ...
        <button className="px-3 py-2 rounded bg-petrol text-white min-h-[44px]" onClick={()=>{ /* explicit search handled by debounce */ }} disabled={loading}>Search</button>
      </div>
            ...
            {items.clients.map(c => (
              <div key={c.id} className="py-2 flex items-center justify-between">
                ...
                <div className="flex gap-2 text-sm">
                  <button className="text-petrol underline min-h-[44px] px-2 inline-flex items-center" onClick={()=>onSelectClient && onSelectClient(c.name)}>Open Card</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {(q && items.clients.length===0 && !loading) && (
        <div className="mt-3 text-sm text-slate-600">No client found. <button className="text-petrol underline min-h-[44px] px-2 inline-flex items-center" onClick={()=>onCreateClient && onCreateClient(q)}>Create client</button></div>
      )}
    </div>
  )
}
